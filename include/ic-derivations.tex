...
这意味着，如果 $n$ 是~$!A$ 的一个证明的哥德尔数，那么 $\Th{Q} \Proves
\OPrf(\num{n}, \gn{!A})$，否则 $\Th{Q} \Proves
\lnot\OPrf(\num{n}, \gn{!A})$。
类似地，如果 $n$ 是 $\lnot !A$ 的一个证明的哥德尔数，那么 $\Th{Q} \Proves \ORefut(\num{n},
\gn{!A})$，否则 $\Th{Q} \Proves \lnot\ORefut(\num{n},
\gn{!A})$。
我们利用对角线引理找到一个语句 $!R$，使得 $\Th{Q} \Proves !R \liff \lnot \ORProv(\gn{!R})$。
Rosser（罗瑟）定理断言 $\Th{T} \Proves/ !R$ 且 $\Th{T} \Proves/ \lnot !R$。
...
\Proves !R$（有一个推导~$\delta$），并证明这将导致 $\Th{T}$ 不一致。令 $n$ 为~$\delta$ 的哥德尔数。由于 $\OPrf$ 在~$\Th{Q}$ 中表示证明关系，所以存在 $\OPrf(\num{n},
\gn{!R})$ 的一个推导~$\delta_1$。此外，由于假定 $\Th{T}$ 是一致的，没有 $k < n$ 是~$!R$ 的反驳的哥德尔数，因此对每个 $k < n$，都有 $\Th{Q} \Proves \lnot \ORefut(\num{k}, \gn{!R})$；令
...
现在证明 $\Th{T} \Proves/ \lnot !R$。同样，假设如此。那么就存在 $\lnot !R$ 的一个推导 $\rho$，其哥德尔数为 $m$——即~$!R$ 的一个反驳——从而由推导~$\rho_1$ 可得 $\Th{Q} \Proves
\ORefut(\num{m}, \gn{!R})$。由于我们假设 $\Th{T}$ 是一致的，故 $\Th{T} \Proves/ !R$。所以对所有 $k$，$k$ 都不是~$!R$ 的推导的哥德尔数，因此由推导~$\pi_k$ 可得 $\Th{Q} \Proves \lnot
\OPrf(\num{k}, \gn{!R})$。于是我们有：
